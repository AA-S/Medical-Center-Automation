	...
	<body>
		...
		<?php
			...
			if (isset($_SESSION['patient']) || isset($_COOKIE['user'])) 
			{
		 	...
		 	//echo $profile;
    	 	$query = "SELECT * FROM `patients_table` WHERE `email`='$profile'";
		...
		?>
			...
			onMouseOver="document.all.test.stop()" onMouseOut="document.all.test.start()">
			 <h1 style="color:red;">Welcome <span style="color:white;font-style:italic;"> <?php echo " ".$row['firstName']." ".$row['lastName']." "; ?></span> to Our KUET Online Medical Center Service</h1></marquee>
			 </div>
		...
		?>
			...
			<marquee id="test" behavior="scroll" direction="left" height="40" scrolldelay="3" scrollamount="6" style="padding:10px;margin-left:200px;margin-right:200px;" 
			onMouseOver="document.all.test.stop()" onMouseOut="document.all.test.start()">
			 <h1 style="color:red;">Welcome <span style="color:white;font-style:italic;"> <?php echo " ".$row['name']." "; ?></span> to Our KUET Online Medical Center Service</h1></marquee>
			 </div>
